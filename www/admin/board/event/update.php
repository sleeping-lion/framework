...
try {
	...
	$setEventRequestType=new SetEventRequestType($_POST);	
	$setContent->update($setEventRequestType);
	
	if($_FILES['photo']['size']) {
		require_once $setContentPhotoClassPath;
		$setEventPhoto=new SetEventPhoto($con);
		$setEventPhoto->delete(new SetEventPhotoRequestType($setEventRequestType->id));
		$setEventPhoto->insert(new SetEventPhotoRequestType($setEventRequestType->id,$_FILES['photo']));
	}

...
try {
	...
	$setAfterRequestType=new SetAfterRequestType($_POST);
	$setContent->update($setAfterRequestType);

	if($_FILES['photo']['size']) {
		require_once $setContentPhotoClassPath;
		$setContentPhoto=new SetAfterPhoto($con);
		$setContentPhoto->delete(new SetAfterPhotoRequestType($setAfterRequestType->id));
		$setContentPhoto->insert(new SetAfterPhotoRequestType($setAfterRequestType->id,$_FILES['photo']));
	}

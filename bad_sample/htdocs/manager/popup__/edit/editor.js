...
this.fontnames = {
    "πŸ≈¡":             "πŸ≈¡, πŸ≈¡√º serif",
    "∞ÌµÒ":             "∞ÌµÒ, ∞ÌµÒ√º sans-serif",
    "±º∏≤":             "±º∏≤, ±º∏≤√º sans-serif",
    "ªı±º∏≤":           "ªı±º∏≤, ªı±º∏≤√º sans-serif",
    "µ∏øÚ":             "µ∏øÚ, µ∏øÚ√º serif",
    "±√º≠":             "±√º≠, ±√º≠√º serif",
...
this.stylesheet = "/css/style.css"; // full URL to stylesheet
...
function editor_generate(objname,userConfig,up_Url) {
...
function editor_action(button_id) {
  ...
  // help popup
  if (cmdID == 'showhelp') {
    window.open(_editor_url + 'popups/editor_help.html', 'EditorHelp', 'scrollbars=no,resizable=yes,top=80, left=80, width=620, height=420');
    return;
  }
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.
  else {
    ...
    // insert image
      ...
      mObject.editdoc = editdoc;
      mObject.config = config;

      showModalDialog(_editor_url + "popups/upload_image.php?upUrl="+_Upload_url, mObject, "resizable: no; help: no; status: no; scroll: no; ");
    }
  ...
  // catch keypress events
    if (editEvent && editEvent.keyCode) {
      ...
      if (ord == 18) { return; }  // ignore alt key by itself
	   ...
	   // cancel ENTER key and insert   replace nextlines with breaks
	   //  if (ord == 13 && editEvent.type == 'keypress') {     // replace nextlines with breaks
	//		var contents = document.all[objname].value;
	//		contents = contents.replace(/\r\n/g, '<br>');
			//contents = contents.replace(/\n/g, '<br>');
			//contents = contents.replace(/\r/g, '<br>');
